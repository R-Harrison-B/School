...
public class ExceptionalLibrary {
  ...
  public int parsePinCode(String s, int errorOffset) throws ParseException {
    int returnCode = 0;
    try {
      returnCode = Integer.parseInt(s);
    } catch (NumberFormatException e) {
      throw new ParseException("Error Pin code must be a number", errorOffset);
    }
    if (returnCode >= 9999 || returnCode <= 1000) {
      throw new ParseException("Error pin code must be between 1000 and 9999", errorOffset);
    }
    return returnCode;
  }
  ...
  public int parseCardBarCode(String s, int errorOffset) throws ParseException {
    ...
    try {
      ...
      throw new ParseException("Error Bar code must be a number", errorOffset);
    }
    if (!(Subscriber.checkCardBarCode(returnCode))) {
      throw new ParseException("Error Card Bar Code is incorrect", errorOffset);
    }
    return returnCode;
  }
  ...
  public void parseRunSubscriberUpdatePhoneNumberCommand(String[] commands, Subscriber subscriber)
    ...
    checkCommandArgumentsCount(commands, 2);    
    try {
      Long.parseLong(commands[1]); // parse the String argument s as a phone number (as long number)
    } catch (NumberFormatException e) { // syntax error
      throw new ParseException("Error: The phone number MUST be a NUMBER.\n", 1);
    }    
  ...
  public void parseRunLibrarianSaveBooksCommand(String[] commands) throws ParseException {
    ...
    try {
      writer = new PrintWriter(new FileOutputStream(commands[1]));
  ...
  public void parseRunLibrarianLoadBooksCommand(String[] commands) throws ParseException {
    ...
    this.checkCommandArgumentsCount(commands, 2);
    try {
      String one = loadFiles(commands[1]);
   ...
   * @return
   */
  private String loadFiles(String fileName) throws ParseException {
    String file;
    String[] newBooks;
    String problem = "good";
    ...
    try {
      ...
      while (scanner.hasNextLine()) {
        ...
        if (!(file.equals(""))) {
          newBooks = file.trim().replaceAll("\\s+", " ").split(":");
        } else {
          continue;
        }
        if (file.equals(":")) {
        ...
        if (newBooks.length > 1 && !(newBooks[0] == null) && !(newBooks[0] == "")
            && !(newBooks[1] == null) && !(newBooks[1] == "")) {
          this.books.add(new Book(newBooks[0], newBooks[1]));
        } else {
          problem = file;
        }
      ...
      System.out.println("Error: Could NOT load books contents from file " + fileName);
    } finally {
      scanner.close();
    }
    if (!(problem.equals("good"))) {
      throw new ParseException(
          "Error: Found incorrectly formatted line in file " + fileName + " " + problem, 0);
    }
    return problem;
  }
  ...
  public void parseRunLoginAsLibrarian(String[] commands, Scanner scanner) throws ParseException {
    // TODO Modify this method to check for its syntax (number of arguments for instance)
    try {
      checkCommandArgumentsCount(commands, 2);
    } catch (ParseException e) {
      throw new ParseException("error message", 0);
    }
    if (commands[1] == null) {
      throw new ParseException("error message", 0);
    }
